...
- 每次成功拍照自动把定位地址追加进"本周"汇总表（CSV），跨周自动新建下一份
...
每次成功保存留痕照片时，程序会把本次定位自动追加进"本周"的汇总表（CSV 格式，UTF-8 带 BOM，Excel/WPS 直接打开不乱码）；跨周自动新建下一份，无需配置。

文件位置：本地存储目录根目录，命名 `FieldTrace_周汇总_<年份>-W<ISO周号>.csv`
...
| 纬度 / 经度 | 坐标（6 位小数）|
| 精度(米) | 定位精度 |
| 定位来源 | wifi-native / ip-whois 等 |
| 质量 | G(绿) / M(黄) / B(红) |
| 地址 | 逆地理地址；解析失败时填 `无地址(来源)` |
